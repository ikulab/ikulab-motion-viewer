...
void Base::initWindow() {
    ...
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "First Triangle Base", nullptr, nullptr);
...
void Base::createTextureImage() {
    ...
    VkDeviceSize imageSize = txtWidth * txtHeight * 4;
    ...
    mipLevels = static_cast<uint32_t>(
        std::floor(std::log2(std::max(txtWidth, txtHeight))) + 1
        );
...
void Base::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t txtWidth, int32_t txtHeight, uint32_t mipLevels) {
    ...
    int32_t mipWidth = txtWidth;
    int32_t mipHeight = txtHeight;
